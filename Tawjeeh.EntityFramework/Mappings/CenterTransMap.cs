...
{
    public  class CenterTransMap : EntityTypeConfiguration<CenterTrans>
    {
        public CenterTransMap()
        {
            HasKey(a => a.Id);
            ...
            ToTable("tblCenterTrans");
            // Relationships
            this.HasOptional(t => t.Centers).WithMany(t => t.CenterTrans).HasForeignKey(d => d.CenterId).WillCascadeOnDelete(false);
            this.HasOptional(t => t.Emirates).WithMany(t => t.CenterTrans).HasForeignKey(d => d.EmiratesId).WillCascadeOnDelete(false);
        }
    }
}
